...
async def T2_Run(): 
    ...
    await RL.driveRobotAndLift(-290,400,250,"left",300,900,"coast",False,True,waitForPrompt)
    await RL.moveAttachment("left",450,950,False,"hold",False,True,waitForPrompt)
    await RL.turnRobot(-106,200,200,False,waitForPrompt)
    await RL.driveRobot(260,150,100,"hold",waitForPrompt)
    await RL.wait(250)
    await RL.driveRobot(-160,200,200,"hold",waitForPrompt)
    
    if RL.debugL1: print("   Drive to right launch area")
    await RL.turnRobot(50,270,300,"hold",waitForPrompt)
    await RL.driveRobot(-400,500,450,"hold",waitForPrompt)
    await RL.turnRobot(78,270,300,"hold",waitForPrompt)
    await RL.driveRobotAndLift(850,800,450,"left",1850,900,"hold",False,True,waitForPrompt)
    await RL.turnRobot(16,200,200,"hold",waitForPrompt)
    await RL.driveRobot(850,900,450,"hold",waitForPrompt)
