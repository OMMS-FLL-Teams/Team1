...
async def T4_Run(): 
    ...
    await RL.moveAttachment("right",-148,300,False,"hold",False,True, waitForPrompt)
    await RL.wait(500)
    await RL.turnRobot(-27,300,400,"hold",waitForPrompt)
    await RL.wait(500)
    await RL.turnRobot(9,300,400,"hold",waitForPrompt)
    await RL.moveAttachment("right",-49,500,False,"brake",False,True, waitForPrompt)
    await RL.driveRobot(15,200,500,"hold",waitForPrompt)
    await RL.moveAttachment("right",-1,500,False,"hold",False,True, waitForPrompt)
    await RL.driveRobot(15,200,500,"hold",waitForPrompt)

    if RL.debugL1: print("   tip the scales - bucket")
    await RL.turnRobot(-48,300,400,"hold",waitForPrompt)
    await RL.driveRobot(290,600,500,"hold",waitForPrompt)
    await RL.turnRobot(-79,300,400,"hold",waitForPrompt)
    await RL.driveRobot(80,300,500,"hold",waitForPrompt)
