...
async def T4_Run(): 
    ...
    await RL.driveRobot(525,800,500,"hold",waitForPrompt)
    ...
    await RL.turnRobot(-22,300,400,"hold",waitForPrompt)
    await RL.driveRobot(45,800,500,"hold",waitForPrompt)
    await RL.moveAttachment("right",-146,300,False,"hold",False,True, waitForPrompt)
    ...
    await RL.driveRobot(20,800,500,"hold",waitForPrompt)

    if RL.debugL1: print("  what's on sale - red paddle")
    await RL.turnRobot(-46,300,400,"hold",waitForPrompt)
    await RL.driveRobot(430,600,500,"hold",waitForPrompt)
    await RL.turnRobot(15,300,400,"hold",waitForPrompt)
    await RL.driveRobot(-215,600,500,"hold",waitForPrompt)

    
    if RL.debugL1: print("   tip the scales - bucket")        
    await RL.driveRobot(70,800,600,"hold",waitForPrompt)
    await RL.turnRobot(-110,400,400,"hold",waitForPrompt)
    await RL.moveAttachment("right",-150,300,False,"hold",False,True, waitForPrompt)
    await RL.moveAttachment("right",0,300,False,"hold",False,True, waitForPrompt)
    await RL.turnRobot(-80,400,400,"hold",waitForPrompt)
    await RL.driveRobot(350,800,600,"hold",waitForPrompt)
    await RL.turnRobot(49,400,400,"hold",waitForPrompt)
    await RL.driveRobot(570,800,600,"hold",waitForPrompt)
